...
async function changeScreen(id) {
  ...
  //   console.log("ALL DATA", items);  // This will log all the data stored in chrome.storage.local
  // });
  // Global resetting
  checkQR.stop();
  inSettings = false;
...
document.getElementById("deleteButton").onclick = async () => {
  ...
  showDeleteModal(`Are you sure you want to delete this device (${data.name})? You'll need to delete it from your Duo account too.`, async () => {
    ...
    await chrome.storage.local.remove(data.activeDevice);
    // Remove from devices
...
async function getExportableData() {
  ...
  let allDevices = await new Promise((resolve) => chrome.storage.local.get(info.devices, resolve)); // so ig you can grab multiple storage values at once
...
document.getElementById("exportTOTPButton").addEventListener("click", async function () {
  let info = await getDeviceInfo();
  let allDevices = await new Promise((resolve) => chrome.storage.local.get(info.devices, resolve));
  let totps = [];
...
async function updatePage(deviceInfo) {
  ...
  let allDevices = await new Promise((resolve) => chrome.storage.local.get(deviceInfo.devices, resolve));
